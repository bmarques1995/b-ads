...
#ifdef B_ADS_ANIMATIONS_ENABLED
...
void SectionTitleWidget::mouseReleaseEvent(QMouseEvent* ev)
{
	...
	if (_fw)
	{
		...
		if (sw)
		{
			cw->_dropOverlay->setAllowedAreas(B_ADS_NS::AllAreas);
			DropArea loc = cw->_dropOverlay->showDropOverlay(sw);
			if (loc != InvalidDropArea)
			{
#if !defined(B_ADS_ANIMATIONS_ENABLED)
				...
				moveAnim->setDuration(B_ADS_ANIMATION_DURATION);
				...
				resizeAnim->setDuration(B_ADS_ANIMATION_DURATION);
...
void SectionTitleWidget::mouseMoveEvent(QMouseEvent* ev)
{
	...
	if (_fw && (ev->buttons() & Qt::LeftButton))
	{
		...
		const QPoint moveToPos = ev->globalPos() - (_dragStartPos + QPoint(B_ADS_WINDOW_FRAME_BORDER_WIDTH, B_ADS_WINDOW_FRAME_BORDER_WIDTH));
		_fw->move(moveToPos);
		...
		if (true)
		{
			...
			{
				cw->_dropOverlay->setAllowedAreas(B_ADS_NS::TopDropArea);
			...
			{
				cw->_dropOverlay->setAllowedAreas(B_ADS_NS::RightDropArea);
			...
			{
				cw->_dropOverlay->setAllowedAreas(B_ADS_NS::BottomDropArea);
			...
			{
				cw->_dropOverlay->setAllowedAreas(B_ADS_NS::LeftDropArea);
