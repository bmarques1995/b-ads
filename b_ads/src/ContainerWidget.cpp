...
QByteArray ContainerWidget::saveState() const
{
	...
	{
		writer.write(B_ADS_NS_SER::ET_Hierarchy, hierarchyData);
	}
...
bool ContainerWidget::restoreState(const QByteArray& data)
{
	...
	B_ADS_NS_SER::InMemoryReader reader(data);
	if (!reader.initReadHeader())
		return false;

	// Basic hierarchy data.
	QByteArray hierarchyData;
	if (reader.read(B_ADS_NS_SER::ET_Hierarchy, hierarchyData))
	{
...
{
	B_ADS_Expects(targetSection != NULL);
...
bool ContainerWidget::saveSectionIndex(B_ADS_NS_SER::SectionIndexData& sid) const
{
	if (_sections.count() <= 0)
		return false;
